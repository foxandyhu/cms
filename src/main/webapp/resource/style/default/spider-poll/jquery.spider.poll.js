(function ($) {
    var defaults = {
        ...
        itemStyle: {
            ...
            bgColor: '#ffffff'//每条的背景颜色
        }
    };
    $.fn.poll = function (id, options) {
        options = $.extend(defaults, options);
        var data = options.data;//数据JSON
        var o = this;
        var total = 0;
        $.each(data, function (i, n) {//统计一共的投票数
        ...
        $.each(data, function (i, n) {
            ...
            if (imgWidth > 0) {
                if (i > (options.itelTotal - 1))
                    ...
                    index = i;
                itemDiv = "<div style='width:80%;background-color:" + options.itemStyle.bgColor + ";font-size:" + options.itemStyle.fontSize + "'><div divWidth='" + imgWidth + "' style='float:left;width:0%;background-image:url(" + options.bgImg + ");' class='poll_plan" + index + "' ><div class='plan_e' style='background-image:url(" + options.bgImg + ");'><div class='plan_c'  style='background-image:url(" + options.bgImg + ");'></div></div></div>"+"</div>"+"<div class='item-percent'>"+n.value + "票&nbsp;(" + percentage + "%)"+"</div>";
            } else {
                itemDiv = '';
            }
            $("table", o).append("<tr><td width='10%' align='right' >" + n.name + ":</td><td width='90%' bgcolor='" + options.itemStyle.bgColor + "' >" + itemDiv + "</td></tr>");
