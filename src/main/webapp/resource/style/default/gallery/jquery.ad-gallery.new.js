...
//      );
    ...
    _afterShow: function() {
      this.gallery_info.html("<i>"+(this.current_index + 1) +'</i> / '+ this.images.length);
    ...
    _showWhenLoaded: function(index, callback) {
      if(this.images[index]) {
        ...
        var img = $(new Image()).attr('src', image.image);
          this.settings.description_wrapper.html(this.images[index].title);
        //以下添加部分
    ...
    jumpLocation: function(index){
		...
		$(".ad-info").html("<i>"+(parseInt(index)+1)+"</i> / "+count_length);
