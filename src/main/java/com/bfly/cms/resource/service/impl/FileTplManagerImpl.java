...
public class FileTplManagerImpl implements TplManager {
    ...
    public List<Tpl> getListByPrefix(String prefix) {
        ...
        if (parent.exists()) {
            ...
            if (files != null) {
                List<Tpl> list = new ArrayList<>();
                for (File file : files) {
                    list.add(new FileTplImpl(file, root));
                }
                return list;
            } else {
    ...
    public List<Tpl> getChild(String path) {
        ...
        if (child != null) {
            List<Tpl> list = new ArrayList<>(child.length);
            for (File f : child) {
                list.add(new FileTplImpl(f, root));
            }
            return list;
        } else {
            return new ArrayList<>(0);
