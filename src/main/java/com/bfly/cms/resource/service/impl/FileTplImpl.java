...
public class FileTplImpl implements Tpl {
	private File file;
	// 应用的根目录
	private String root;
	private List<FileTplImpl> child;

	public FileTplImpl(File file, String root) {
		this.file = file;
		this.root = root;
	}
	 ...
	 * @return
	 */
	public List<FileTplImpl> getChild() {
		if (this.child != null) {
			return this.child;
		}
		File[] files;
		files = getFile().listFiles();
		List<FileTplImpl> list = new ArrayList<FileTplImpl>();
		if (files != null) {
			Arrays.sort(files, new FileComparator());
			for (File f : files) {
				FileTplImpl fw = new FileTplImpl(f, root);
				list.add(fw);
			}
		}
		return list;
	 ...
	 * @param child
	 */
	public void setChild(List<FileTplImpl> child) {
		this.child = child;
	}
}
