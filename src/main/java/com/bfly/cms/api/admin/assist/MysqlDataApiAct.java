...
import com.bfly.cms.manager.assist.CmsMysqlDataBackMng;
import com.bfly.cms.manager.assist.CmsResourceMng;
import com.bfly.common.file.FileWrap;
...
public class MysqlDataApiAct {
    ...
    public void files(HttpServletRequest request, HttpServletResponse response) {
        List<FileWrap> list = resourceMng.listFile(com.bfly.cms.Constants.BACKUP_PATH, false);
        JSONArray jsonArray = new JSONArray();
        if (list != null && list.size() > 0) {
            for (int i = 0; i < list.size(); i++) {
                jsonArray.put(i, list.get(i).convertToJson());
    ...
    public void revert(String filename, String db, HttpServletResponse response, HttpServletRequest request) {
        ...
        if (!errors.hasErrors()) {
            String backpath = realPathResolver.get(com.bfly.cms.Constants.BACKUP_PATH);
    ...
    public void rename(String origName, String distName, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (!errors.hasErrors()) {
            String orig = com.bfly.cms.Constants.BACKUP_PATH + "/" + origName;
            String dist = com.bfly.cms.Constants.BACKUP_PATH + "/" + distName;
        ...
        if (!errors.hasErrors()) {
            ...
            if (!errors.hasErrors()) {
                ...
                if (nameArr != null && nameArr.length > 0 && nameArr[0] != null) {
                    backName = nameArr[0].substring(nameArr[0].indexOf(com.bfly.cms.Constants.BACKUP_PATH)
                            + com.bfly.cms.Constants.BACKUP_PATH.length() + 1);
                }
    ...
    public void getOrigName(String name, HttpServletRequest request, HttpServletResponse response) {
        ...
        if (!errors.hasErrors()) {
            String origName = name.substring(com.bfly.cms.Constants.BACKUP_PATH.length());
    ...
    private boolean validate(String[] names, HttpServletRequest request) {
        if (names != null && names.length > 0) {
            for (String name : names) {
                //导出阻止非法获取其他目录文件
                if (!name.contains(com.bfly.cms.Constants.BACKUP_PATH)
    ...
    private class DateBackupTableThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                writer.write(com.bfly.cms.Constants.ONESQL_PREFIX + DISABLEFOREIGN);
                for (int i = 0; i < tablenames.length; i++) {
                    backup_table = tablenames[i];
                    backupTable(writer, tablenames[i]);
                }
                writer.write(com.bfly.cms.Constants.ONESQL_PREFIX + ABLEFOREIGN);
                backup_table = "";
                writer.close();
                out.close();
            } catch (IOException e) {
        ...
        private String createOneTableSql(String tablename) {
            ...
            Object[] oneResult;
            buffer.append(com.bfly.cms.Constants.ONESQL_PREFIX + "DROP TABLE IF EXISTS "
                    + tablename + BRANCH + BR);
            buffer.append(com.bfly.cms.Constants.ONESQL_PREFIX
                    + dataBackMng.createTableDDL(tablename) + BRANCH + BR
                    + com.bfly.cms.Constants.ONESQL_PREFIX);
        ...
        private String createOneInsertSql(Object[] oneResult, String tablename) {
            StringBuffer buffer = new StringBuffer();
            buffer.append(com.bfly.cms.Constants.ONESQL_PREFIX + INSERT_INTO + SPLIT + tablename
