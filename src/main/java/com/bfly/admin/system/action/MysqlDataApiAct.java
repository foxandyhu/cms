...
import com.bfly.cms.system.service.CmsMysqlDataBackMng;
import com.bfly.admin.resource.action.CmsResourceApiAct;
...
public class MysqlDataApiAct {
    ...
    public void files(HttpServletRequest request, HttpServletResponse response) {
        List<FileWrap> list = resourceMng.listFile(Constants.BACKUP_PATH, false);
        JSONArray jsonArray = new JSONArray();
        if (list != null && list.size() > 0) {
            for (int i = 0; i < list.size(); i++) {
                jsonArray.put(i, list.get(i).convertToJson());
        ...
        if (!errors.hasErrors()) {
            ...
            if (!errors.hasErrors()) {
                ...
                if (nameArr != null && nameArr.length > 0 && nameArr[0] != null) {
                    backName = nameArr[0].substring(nameArr[0].indexOf(Constants.BACKUP_PATH)
    ...
    private boolean validate(String[] names, HttpServletRequest request) {
        if (names != null && names.length > 0) {
            for (String name : names) {
                //导出阻止非法获取其他目录文件
                if (!name.contains(Constants.BACKUP_PATH)
